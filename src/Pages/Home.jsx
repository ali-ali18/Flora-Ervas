...
const Home = () => {
	const { posts, loading } = useFetchPosts(6); // Busca os posts de forma otimizada
	const navigate = useNavigate();

	// Memoização para evitar renderizações desnecessárias
	const memoizedCategorias = useMemo(() => categorias, []);
	...
	return (
		<main className="px-4 md:px-16 py-2 pb-12 bg-slate-100 min-h-screen">
			<Helmet>
				...
				/>
				{memoizedCategorias.map((img, index) => (
					<link key={index} rel="preload" as="image" href={img.img} />
				))}
			</Helmet>

			{/* Lazy-loading principal */}
			<Suspense fallback={<Spinner label="Carregando..." color="success" />}>
				{/* Banner inicial */}
				<BannerPrincipal />
				...
				<section className="flex justify-center w-full mt-0 flex-col">
					...
					<div className="flex gap-3 overflow-x-auto p-5 scrollbar-hide">
						{memoizedCategorias.map((categoria) => (
							<CardImg
